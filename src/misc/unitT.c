...
bool testFullCollisionScenario(){
  ...
  if(!handleCollision(p1, p2,0,arr)){
    move(p1,1);
    move(p2,1);
  }
...
bool testLennardJones(){
  ...
  lennardJonesPotentialForce(p1,p2,response);
  ...
  lennardJonesPotentialForce(p1,p2,response2);
  ...
  for (int i = 0; i < 10; ++i)
  {
    lennardJonesPotentialForce(p1,p2,response3);
...
bool testElectroStatic(){
  ...
  electroStaticForce(p1, p2, response);
  ...
  {
    electroStaticForce(p1,p2,response2);
...
bool testLorentzForce(){
  ...
  {
    applyFieldAcceleration(p1, elFi->fieldVector, magFi->fieldVector);
    move(p1,0.001);
  }
...
bool testForceApplication(){
  ...
  double pos4[] = {1,0,0};
  double vel4[] = {0,0,0};
  struct particle *parr[] = {p1,p2};

  setParticleInfo(p1,pos3,vel3,acc,att1);
  setParticleInfo(p2,pos4,vel4,acc,att2);

  applyInterAcceleration(p1, parr , 0, 2);
